            ...
            if n > 1:
                ...
                self.carRepairDict.update(
                    {str(workOrderCode): {"workOrderCode": workOrderCode, "customCustomName": customCustomName,
                                          ...
                                          "WorkOrderMemo": WorkOrderMemo,
                                          "workOrderMaintenanceTechnician": workOrderMaintenanceTechnician,
                                          "workOrderDeliveryCarDate": workOrderDeliveryCarDate,
                                          "workOrderPromiseOverDate": workOrderPromiseOverDate,
                                          "msReasonCheckResultWhy": msReasonCheckResultWhy,
                                          "msReasonMBreakdownWhy": msReasonMBreakdownWhy}})
    ...
    def __importCarSettle(self):
        ...
        for rows in sheets[0].get_rows():
            ...
            if n > 1:
                ...
                workOrderPayablePartsMoney = rows[9].value  # 零件應付金額
                workOrderTotalPartsMoney = rows[10].value  # 零件合计金額
                workOrderAllowancePartsMoney = rows[11].value  # 零件折扣金額
                workOrderPayableWorkTimeMoney = rows[12].value  # 工時應付金額
                workOrderTotalWorkTimeMoney = rows[13].value  # 工时合计金額
                workOrderAllowanceWorkTimeMoney = rows[14].value  # 工时折扣金額
                # 救援费[15]
                # 结算日期[16]
                # 结算人[17]
                # 付款方式[18]
                ...
                # 备注[31]
                try:
                    workOrderAllowanceCostMoney = float(workOrderAllowancePartsMoney) + float(
                        workOrderAllowanceWorkTimeMoney)
                except Exception as e:
                    workOrderAllowanceCostMoney = 0
                self.carSettleDict.update({workOrderCode: {"workOrderFinishOverDate": workOrderFinishOverDate,
                                                           "workOrderTotalCostMoney": workOrderTotalCostMoney,
                                                           "workOrderPayablePartsMoney": workOrderPayablePartsMoney,
                                                           "workOrderTotalPartsMoney": workOrderTotalPartsMoney,
                                                           "workOrderAllowancePartsMoney": workOrderAllowancePartsMoney,
                                                           "workOrderPayableWorkTimeMoney": workOrderPayableWorkTimeMoney,
                                                           "workOrderTotalWorkTimeMoney": workOrderTotalWorkTimeMoney,
                                                           "workOrderAllowanceWorkTimeMoney": workOrderAllowanceWorkTimeMoney,
                                                           "workOrderPayableCostMoney": workOrderPayableCostMoney,
                                                           "workOrderRepairType": workOrderRepairType,
                                                           "workOrderAllowanceCostMoney": workOrderAllowanceCostMoney}})
    ...
    def __importOutbound(self):
        ...
        for rows in sheets[0].get_rows():
            ...
            if n > 1:
                ...
                # 退料[36]
                # 常用件[37]
                itemPickingOrder = str(rows[38].value).split('.')[0] if len(
                    str(rows[38].value).split('.')) > 0 else ""  # 領料單號
                # 分厂[39]
                ...
                self.carOutboundList.append({"workOrderCode": workOrderCode, "itemCode": itemCode, "itemName": itemName,
                                             "itemAmount": itemAmount, "itemOneMoney": itemOneMoney,
                                             "itemMoney": itemMoney, "itemDiscountMoney": itemDiscountMoney,
                                             "itemPickingDate": itemPickingDate,
                                             "itemOneCostMoney": itemOneCostMoney,
                                             "itemPickingTechnician": itemPickingTechnician,
                                             "itemPickingOrder": itemPickingOrder, "itemDiscountRate": itemDiscountRate,
                                             "itemOneSellMoney": itemOneSellMoney})
        ...
        for rows in sheets[0].get_rows():
            ...
            if n > 1:
                ...
                try:
                    ...
                    itemDiscountMoney = 0
                    itemDiscountRate = 1
                self.carHousList.append({"workOrderCode": workOrderCode, "itemCode": itemCode, "itemName": itemName,
                                         "itemStandardWorkTime": itemStandardWorkTime,
                                         "itemMoney": itemMoney, "itemPayType": itemPayType,
                                         "itemTechnician": itemTechnician,
                                         "itemType": itemType, "itemAmount": itemAmount,
                                         "itemDiscountMoney": itemDiscountMoney,
    ...
    def __importCarsUser(self):
        ...
        for rows in sheets[0].get_rows():
            ...
            if n > 1:
                ...
                self.carsUserDict.update({carVIN: {"customPersonName": customPersonName, "customLicence": customLicence,
                                                   "carLicence": carLicence,
                                                   "carVIN": carVIN, "carEngineCode": carEngineCode,
                                                   "delivererPersonName": delivererPersonName,
                                                   "delivererBaSexID": delivererBaSexID,
                                                   "delivererPhone": delivererPhone,
                                                   "delivererAddress": delivererAddress,
                                                   "carSaleInvoiceDate": carSaleInvoiceDate,
                                                   "carWarrantyDate": carWarrantyDate, "carSaleManName": carSaleManName,
                                                   "carInsuranceCompany": carInsuranceCompany,
                                                   "carPeriodicMaintenanceDate": carPeriodicMaintenanceDate,
                                                   "memo": memo,
...
def getFiles(deskPath):
    """
    获取指定目录的所有导出文件
    :return:
    """
    path = deskPath  # ex保存在桌面
    files = os.listdir(path)
    carRepairExfile = False
    carSettleExfile = False
    carOutboundExfile = False
    carHousExfile = False
    carsUserExfile = False
    for file in files:
        if '维修查询20' in file:
    ...
    if all([carRepairExfile, carSettleExfile, carOutboundExfile, carHousExfile, carsUserExfile]):
        return carRepairExfile, carSettleExfile, carOutboundExfile, carHousExfile, carsUserExfile
    else:
        print('execl文件有问题....')


def montiCarsExportClass():
    """监控导出程序错误"""
    red = CarsExportClass(sDate, user, pwd, dmsPath, deskPath)  # 导出Ex
    if red.status:
        return True
    else:
        return False
...
if __name__ == "__main__":
    ...
    print('请把分辨率调至1440*900 - 1920*1080 之间，\n配置DMS启动程序路径与桌面路径再运行...')
    # os.system('pause')
    conf = ConfigParser()
    ...
    # threadNum = conf.get('thread', 'threadNum')
    # print(signCode,apiUrl)
    # 判断log文件是否村存在
    if not os.path.exists(deskPath + 'log'):
        os.mkdir(deskPath + 'log')
    ...
    log = LogClass('exProce.log', 'exProce')

    """
    # signCode = '6451135'
    # apiUrl = 'http://data.gt7.com.cn:9090/GBOXData10/action'
    # carRepairExfile = 'ex/维修查询20190419100246.xls'
    # carSettleExfile = 'ex/车辆结算明细20190419091152.xls'
    # carOutboundExfile = 'ex/出库明细20190419090952.xls'
    # carHousExfile = 'ex/项目查询2019-04-19 09：04.xls'
    # carsUserExfile = 'ex/车辆信息20190418174246.xls'
    """
    print('配置文件加载完成...')

    while True:
        time.sleep(5)
        sDate = datetime.date.today()  # 控制导出的日期
        print(sDate - datetime.timedelta(1), '日数据处理开始...')
        try:
            app = CarsExportClass(sDate, user, pwd, dmsPath, deskPath)  # 导出Ex
            print('导出Ex完成...')
            # """
            carRepairExfile, carSettleExfile, carOutboundExfile, carHousExfile, carsUserExfile = getFiles(
                deskPath)  # 导入Ex
            print('导入Ex完成...')
            getWorksCars = GetWorksCarsClass(signCode, apiUrl, carRepairExfile, carSettleExfile, carOutboundExfile,
                                             carHousExfile, carsUserExfile)  # 处理Ex
            print('处理Ex完成...')
            getWorksCars.getCarsList()  # 获取车辆列表 并上传
            print('上传车辆完成...')
            getWorksCars.getWorkList()  # 获取工单列表 并上传
            print('上传工单完成...')
            # """
        except Exception as e:
            print(e)
            log.logger.error('{0}: 循环有问题...{1}'.format(sDate, e))
            continue
        print('等待下次数据处理...')
        time.sleep(20000)
