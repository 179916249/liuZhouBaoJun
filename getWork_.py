    ...
    def __importCarSettle(self):
        ...
        for rows in sheets[0].get_rows():
            ...
            if n > 1:
                ...
                # 备注[31]
                try:
                    workOrderAllowanceCostMoney = float(workOrderAllowancePartsMoney)+float(workOrderAllowanceWorkTimeMoney)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        try:
            CarsExportClass(sDate, user, pwd, dmsPath, deskPath)  # 导出Ex
            print('导出Ex完成...')
            # """
            carRepairExfile, carSettleExfile, carOutboundExfile, carHousExfile, carsUserExfile = getFiles(deskPath)     # 导入Ex
            print('导入Ex完成...')
